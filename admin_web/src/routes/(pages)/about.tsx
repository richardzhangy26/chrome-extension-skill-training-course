            ...
            <div className="grid gap-8 sm:grid-cols-2">
              ...
              <div className="flex items-center gap-8">
                <Avatar className="size-32 p-0.5">
                  <AvatarImage className="border-border rounded-full" src={logo} alt={m.about_avatar()} />
                  <AvatarFallback className="absolute inset-0">
                    <div className="text-muted-foreground size-32" />
                  </AvatarFallback>
                </Avatar>
                <div>
                  <h1 className="text-3xl font-bold tracking-tight">{websiteConfig.metadata?.name}</h1>
                  <p className="text-muted-foreground mt-2 text-base">{m.about_bio()}</p>
                </div>
              </div>

              {/* Introduction and social */}
              <div>
                <p className="text-muted-foreground mb-8 text-base">{m.about_introduction()}</p>
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  {supportEmail && (
                    <a href={supportEmail} className={cn(buttonVariants(), 'inline-flex items-center rounded-lg')}>
