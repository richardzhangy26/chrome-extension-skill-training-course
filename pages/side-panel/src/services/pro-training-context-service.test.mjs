...
test('buildParticipantRoles: 剔除 user，查表命中，缺失跳过', () => {
  ...
  assert.deepEqual(roles, [
    { nid: 'r1', roleName: '小研', nickname: '研究员', description: '负责分析' },
    { nid: 'r2', roleName: '大鲜', nickname: '鲜哥', description: '负责采购' },
  ]);
  // 不泄露 prompt
...
test('assembleProContext: 按 nid 建 stagesById，字段映射，空值成空串', () => {
  ...
  assert.deepEqual(s1.participantRoles, [{ nid: 'r1', roleName: '小研', nickname: '研究员', description: '分析' }]);
...
test('toStagePromptContext: 扁平化并带上任务级字段', () => {
  const stage = {
    ...
    participantRoles: [{ nid: 'r1', roleName: '小研', nickname: '研究员', description: '分析' }],
  };
